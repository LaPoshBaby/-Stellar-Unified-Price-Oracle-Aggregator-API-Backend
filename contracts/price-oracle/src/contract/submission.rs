...
//
// Includes the Issue #385 replay / DoS hardening for the batch path:
//   - nonce must equal the current BatchNonce (replay of a consumed nonce
//     fails with BatchNonceMismatch)
//   - each leaf of a committed batch can be applied at most once
//     (BatchEntryAlreadyApplied), so the permissionless apply path cannot be
//     used to spam duplicate entries into history
//   - proof depth is capped (merkle::MAX_PROOF_SIBLINGS) and committed roots
//     are pruned after storage::RETAINED_BATCH_ROOTS batches, bounding both
//     per-call gas and total batch bookkeeping storage
...
pub(crate) fn submit_batch(
    ...
    // Issue #385 — nonce monotonicity: the batch nonce only ever moves
    // forward, so replaying a previously committed batch (same nonce) is
    // rejected here.
...
pub(crate) fn apply_batch_entry(
    ...
    // Only valid proofs touch the applied-leaf tracker; duplicates are
    // rejected so history cannot be polluted with repeated entries.
